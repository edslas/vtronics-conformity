...
double Reading; 
	...
	if ((pSetup = LoadPanel (0, "main.uir", SETUP)) < 0)
		return -1;
...
int BuildVNumberList(void)
{
	...
	while(result == 0)
	{
		...
		{
			// add to control
			ierror = InsertListItem (pSetup, SETUP_VNUMBER, i, pVEntry->pVNumber, (unsigned int)pVEntry);		
			if(ierror == 0)
			{
				if(i == 0)
				{
...
VNumberStruct VData;
	...
	switch (event)
		...
		case EVENT_VAL_CHANGED:
			ierror = GetCtrlIndex (pSetup, SETUP_VNUMBER, &i);
			if(ierror == 0)
			{
				ierror = GetValueFromIndex (pSetup, SETUP_VNUMBER, i, &uiData);
...
int ierror;
	...
	ierror += SetCtrlVal (pSetup, SETUP_TOLERENCE, pData->dTolerence);
